...
def sparse_vasprun(path: str, force=True):
    ...
    if force:
        stepi = steps[-1]
        dct = {f"{path}": {"structure": stepi["structure"], "force": np.array(stepi["forces"]),
...
def sparse_vasprun_traj(path: str, space=20):
    ...
    for i, stepi in enumerate(steps):
        if i % space == 0 and i != len(steps) - 1:
            dcti = {"structure": stepi["structure"], "force": np.array(stepi["forces"]),
                    "stress": np.array(stepi["stress"]),
                    "energy": stepi["e_wo_entrp"], "potcar_symbol": potcar_label, "incar": vr.incar,
    ...
    stepi = steps[-1]
    dctn1 = {f"{path}": {"structure": stepi["structure"], "force": np.array(stepi["forces"]),
                         "stress": np.array(stepi["stress"]),
                         "energy": stepi["e_wo_entrp"], "potcar_symbol": potcar_label, "incar": vr.incar,
                         "kpoints_kpts": vr.kpoints.kpts, "kpoints_style": vr.kpoints.style.name, "efermi": vr.efermi}}
    dct.update(dctn1)
...
if __name__ == "__main__":
    ...
    res2 = sparse_vasprun(path, force=False)
